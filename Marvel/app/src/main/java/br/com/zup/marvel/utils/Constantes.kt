package br.com.zup.marvel.utils

const val MARVEL_KEY = "MARVEL"
const val USER_KEY = "user key"
const val GAMORA_NOME = "Gamora"
const val GAMORA_DESCRICAO =
    "Gamora foi uma personagem fictícia que apareceu nas histórias em quadrinhos publicadas pela Marvel Comics. Criada por Jim Starlin, a personagem apareceu pela primeira vez em \\\"Strange Tales\\\" #180. Gamora foi a filha adotiva de Thanos, e a última de sua espécie.\"\n"
const val HOMEM_FERRO_NOME = "Homem de Ferro"
const val HOMEM_FERRO_DESCRICAO =
    "\"O Homem de Ferro foi um personagem dos quadrinhos publicados pela Marvel Comics. Sua verdadeira identidade é o empresário e bilionário Tony Stark, que usou armaduras de alta tecnologia no combate ao crime. Foi criado em 1963 pelo escritor Stan Lee, o roteirista Larry Lieber e os desenhistas Jack Kirby e Don Heck\"\n"
const val MULHER_INVISIVEL_NOME = "Mulher Invisível"
const val MULHER_INVISIVEL_DESCRICAO = "A Mulher Invisível, alter-ego de Susan \"Sue\" Richards, é uma super-heroína de histórias em quadrinhos da editora Marvel Comics. Quando foi criada e durante vários anos chamou-se Garota Invisível. No filme do Quarteto Fantástico de 2005 foi interpretada por Jessica Alba e no de 2015, por Kate Mara."
const val TEXT_HELLO = "Olá "
const val TEXT_HOME = " esses são alguns dos personagens da Marvel"
const val LOGIN = "Login"
const val REGISTER = "Registro"

const val INSERT_EMAIL = "Insira seu email"
const val INSERT_PASSWORD = "Insira sua senha"
const val PASSWORD_SMALL_ERROR = "SENHA INVÁLIDA! Senhas devem ter no mínimo 8 caracteres"
const val LOGIN_ERROR = "Não foi possível realizar o login"
const val REGISTER_ERROR = "Não foi possível realizar o registro"
const val NAME_EMPTY_ERROR = "Preencha seu nome"
const val NAME_SMALL_ERROR = "Insira um nome com mais caracteres"
const val EMAIL_EMPTY_ERROR = "Insira seu email"
const val PASSWORD_EMPTY_ERROR = "Insira qual será sua senha"